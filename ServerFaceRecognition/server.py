...
from forms import RegistrationForm, UpdatingForm
...
def update_picture (form_picture, username) :
    ...
    image_name = "Image{}.jpg".format (count)
    path = path + "/" + image_name
...
def update () :
    form = UpdatingForm()
    if form.validate_on_submit () :
        picture_file = update_picture (form.picture.data, form.username.data)
        flash ('Successfuly updated the image of ' + form.username.data, 'success')
        return redirect (url_for ('home'))
